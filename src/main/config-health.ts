...
//
//  Mithril reads its config from %LocalAppData%\hermes\. Users
...
function checkSiblingHermesHomeDrift(profile?: string): ConfigHealthIssue[] {
  ...
  for (const sibling of siblings) {
    ...
    for (const { field, label } of DRIFT_FIELDS) {
      ...
      // (assumption: Mithril is the broken side, the user's
      ...
      if (!winValue && wslValue) {
        issues.push({
          ...
          severity: "warning",
          message: `${label} is set on WSL (${sibling.distro}) but not on the Windows side that Mithril reads.`,
          detail:
            `WSL value (${where}): ${wslMasked}\n` +
            `Windows value: (not set)\n\n` +
            `Mithril reads only ${current.envFile.replace(/\\\.env$/, "")} — your CLI on WSL works, the desktop doesn't, because the value never made it across. Auto-fix copies the WSL value into the Windows-side file.`,
        ...
        issues.push({
          ...
          detail:
            ...
            `Mithril reads the Windows side, so this isn't blocking the desktop. Just a heads-up that your CLI on WSL is missing this value if you also use it there.`,
        ...
        issues.push({
          ...
          detail:
            `Windows value: ${winMasked}\n` +
            `WSL value (${where}): ${wslMasked}\n\n` +
            `Mithril reads only the Windows side. If these were supposed to be the same, copy whichever value is current to the other side. If they're intentionally different, this notice is informational.`,
          locations: [current.envFile, sibling.hermesHome],
          autoFixable: false,
          context: {
            field,
